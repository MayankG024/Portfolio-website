...
export function LoadingScreen({ onLoadingComplete }: LoadingScreenProps) {
  ...
  useEffect(() => {
    const duration = 4500; // 4.5 seconds total loading (much slower for full visibility)
    const steps = 100;
    ...
    const progressTimer = setInterval(() => {
      setProgress(prev => {
        ...
        if (next >= 100) {
          ...
          setTimeout(onLoadingComplete, 1500); // Extended delay to showcase completion
        }
      ...
      <div className="text-center space-y-8 max-w-md w-full px-6">
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-green-600 font-['Jersey_25'] tracking-wide">
            v1.0.0 - Design Engineer Edition
          </p>
        </div>
