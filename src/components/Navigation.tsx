...
export const Navigation = memo(function Navigation({ isDark, onToggleTheme }: NavigationProps) {
  ...
  return (
    <>
      <nav className="w-full border-b-2 border-foreground bg-background sticky top-0 z-50">
        <div className="w-full px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between w-full">
            ...
            {/* Desktop: Right side - Navigation buttons + Theme Switch */}
            ...
            <div className="flex sm:hidden items-center space-x-2 flex-shrink-0">
              ...
              <button
                ...
                aria-label="Toggle dark mode"
                aria-checked={!!isDark}
                className="relative w-12 h-3 rounded-full border-2 border-foreground bg-background overflow-hidden transition-all duration-150 ease-in-out shadow-[2px_2px_0px_0px_hsl(var(--border))]"
                onClick={() => {
                  trackEvent('theme_toggle', { to: isDark ? 'light' : 'dark' });
                  onToggleTheme && onToggleTheme();
                }}
              >
                <span
                  className={`absolute top-0 left-0 h-full rounded-full bg-foreground transition-all duration-200 flex items-center
                             ${isDark ? 'w-full justify-end pr-1' : 'w-1/2 justify-start pl-1'}`}
                >
                  {isDark ? (
                    <Moon className="text-background w-2.5 h-2.5" />
                  ) : (
                    <Sun className="text-background w-2.5 h-2.5" />
                  )}
                </span>
              </button>
              ...
              <button
                ...
                className="flex flex-col justify-center items-center w-8 h-8 border-2 border-foreground bg-background shadow-[2px_2px_0px_0px_hsl(var(--border))] active:shadow-[1px_1px_0px_0px_hsl(var(--border))] active:translate-x-0.5 active:translate-y-0.5 transition-all duration-150 gap-[4px] p-1.5"
                onClick={() => setMenuOpen(o => !o)}
              >
                <span
                  className={`block w-full h-[2px] bg-foreground transition-all duration-200 origin-center ${menuOpen ? 'rotate-45 translate-y-[6px]' : ''}`}
                />
                <span
                  className={`block w-full h-[2px] bg-foreground transition-all duration-200 ${menuOpen ? 'opacity-0 scale-x-0' : ''}`}
                />
                <span
                  className={`block w-full h-[2px] bg-foreground transition-all duration-200 origin-center ${menuOpen ? '-rotate-45 -translate-y-[6px]' : ''}`}
                />
      ...
      {/* Mobile side drawer */}
      <aside
        className={`fixed top-0 right-0 h-full w-64 bg-background border-l-2 border-foreground z-50 sm:hidden
      ...
      >
        ...
        <button
          ...
          className="absolute top-3 right-3 w-8 h-8 border-2 border-foreground bg-background flex items-center justify-center text-foreground font-bold text-xs shadow-[2px_2px_0px_0px_hsl(var(--border))] active:shadow-none active:translate-x-0.5 active:translate-y-0.5 transition-all"
          onClick={() => setMenuOpen(false)}
        >
          ✕
        </button>
